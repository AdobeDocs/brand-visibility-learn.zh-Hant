---
title: AI助理如何使用外部來源和引文
description: 瞭解AI助理如何從訓練資料中回答和擷取即時來源、什麼是引文，以及為什麼有些引文會產生幻覺。
role: Admin, Leader, User
level: Beginner
doc-type: Value Video
duration: 150
last-substantial-update: 2026-09-02T00:00:00Z
jira: KT-22518
source-git-commit: 823cfd29f7816f485995fcb924740a402993fa54
workflow-type: tm+mt
source-wordcount: '154'
ht-degree: 0%

---

# AI助理如何決定何時使用外部來源，以及什麼是引文？

AI助理要麼從訓練資料中回答，要麼先擷取即時來源。 這段影片說明他們如何決定、引文的真實含義以及為什麼引文不是真實的。

## 這部影片是給誰看的？

* 行銷人員和使用者瞭解AI如何產生來源答案
* 試圖瞭解引文來源的團隊

## 視訊內容

在本影片中，您將瞭解：

* 助理回答問題的兩種方式：訓練資料或擷取
* 時效性提示如何觸發即時搜尋和引文
* AI答案中的引文看起來是什麼樣子
* 為什麼有些引文會有幻覺，可能導致404秒
* 如何辨別模型產生文字中的實際擷取來源

>[!VIDEO](https://video.tv.adobe.com/v/3502840/?captions=chi_hant&learn=on){transcript=true}