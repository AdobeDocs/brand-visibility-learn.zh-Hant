---
title: AI答案中的提及與引文
description: 提及內容會在文字中命名您的品牌；引文會以來源連結您的頁面。 瞭解為何引文來自擷取，且是較強的可見度訊號。
feature: Brand Presence
role: Admin, Leader, User
level: Beginner
doc-type: Value Video
duration: 175
last-substantial-update: 2026-09-02T00:00:00.000Z
jira: KT-22523
TQID: 'https://experienceleague.adobe.com/GimNm702CW2KTYszySLq7SqEhg3fVa8cbaQ67enX4Ls'
product_v2:
  - id: d830747e-f8f3-4fce-8eff-d53b333b1639
    internal-label: Brand Visibility
feature_v2:
  - id: c898dfb2-0885-42fb-b2af-b2d756752646
    internal-label: Best practices
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
    internal-label: User
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
    internal-label: Admin
  - id: f8a45b24-4be7-4f1b-909b-60d06b483a20
    internal-label: Leader
source-git-commit: e014c908d0f11498ae1fdb62a96729df68d20b54
workflow-type: tm+mt
source-wordcount: '162'
ht-degree: 0%
---
# 在AI答案中被提及和被引述之間有何差異？

提及內容會在文字中命名您的品牌；引文會以來源連結您的頁面。 此影片說明差異所在，以及引文為何是較強的可見度訊號。

## 這部影片是給誰看的？

* 行銷人員和使用者解譯AI如何代表其品牌
* 團隊區分品牌提及和真實引用

## 視訊內容

在本影片中，您將瞭解：

* 提及內容如下：您的品牌在答案文字中命名
* 引文是什麼：指向您的頁面的連結，可作為支援來源
* 為何提及內容可能來自訓練資料而不進行擷取
* 擷取(RAG)如何產生真正的引文
* 為什麼引文是說明您的品牌被用作證據的較強訊號

>[!VIDEO](https://video.tv.adobe.com/v/3502765/?learn=on){transcript=true}