---
title: Bing網站管理員工具AI效能報表
description: 瞭解Microsoft的全新Bing網站管理員工具AI效能報告如何追蹤您的內容在AI回應中的引用方式，以及這對於地理位置可見度的意義。
solution: Brand Visibility
feature: Brand Presence
role: Admin, Leader, User
level: Beginner
doc-type: Value Video
duration: 403
last-substantial-update: 2026-09-02T00:00:00.000Z
jira: KT-22500
TQID: 'https://experienceleague.adobe.com/Yjc5C2YMJ-4UgHTsOK-XgIUMr-TaYXTNKh3Y4VxzI9Q'
product_v2:
  - id: d830747e-f8f3-4fce-8eff-d53b333b1639
    internal-label: Brand Visibility
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
    internal-label: User
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
    internal-label: Admin
topic_v2:
  - id: cdd65e7e-8839-44a2-bc21-0e03623b5dd1
    internal-label: Optimization
source-git-commit: e014c908d0f11498ae1fdb62a96729df68d20b54
workflow-type: tm+mt
source-wordcount: '214'
ht-degree: 0%
---
# Bing網站管理員工具AI效能報表

瞭解Microsoft在Bing網站管理員工具中的新AI效能報告如何追蹤您的內容在AI產生的回答中被引為來源的頻率，及其對產生式引擎最佳化(GEO)的意義。

## 這部影片是給誰看的？

* 跨搜尋和AI體驗追蹤內容可見度的管理員、主管和使用者
* 專注於地理和AI驅動探索的行銷人員和內容團隊

## 視訊內容

在本影片中，您將瞭解：

* AI效能報表如何將AI可見度分析圖層新增至現有的Bing網站管理員工具
* 其報告的核心量度，包括引用總數、平均引用頁面和頁面層級引用資料
* 報告如何顯示引文背後的扇出查詢，以及為何不顯示基礎提示
* 引文資料與點按數、曝光數和排名等傳統搜尋效能報表有何不同
* 這對您的GEO策略重要的原因，以及它如何補充Adobe Brand Visibility

>[!VIDEO](https://video.tv.adobe.com/v/3502690/?learn=on){transcript=true}

>[!NOTE]
>AI效能報告目前僅涵蓋Bing Copilot和AI摘要中的引用。 其中尚未包含其他AI助理，例如ChatGPT。