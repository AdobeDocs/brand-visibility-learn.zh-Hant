---
title: 將提示對應至角色與對象
description: 將提示對應到真實人物可顯示內容獲勝或失敗的位置，因為整體受眾的平均可見度會隱藏您在AI答案中的真實位置。
feature: Best Practices
role: Admin, Leader, User
level: Beginner
doc-type: Value Video
duration: 148
last-substantial-update: 2026-09-02T00:00:00.000Z
jira: KT-22526
TQID: 'https://experienceleague.adobe.com/SM1lH6ysl9JjrIRBPtlvSwjmt2U3TN1Zm2j9flTteVI'
product_v2:
  - id: d830747e-f8f3-4fce-8eff-d53b333b1639
    internal-label: Brand Visibility
feature_v2:
  - id: c898dfb2-0885-42fb-b2af-b2d756752646
    internal-label: Best practices
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
    internal-label: User
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
    internal-label: Admin
  - id: f8a45b24-4be7-4f1b-909b-60d06b483a20
    internal-label: Leader
source-git-commit: e014c908d0f11498ae1fdb62a96729df68d20b54
workflow-type: tm+mt
source-wordcount: '164'
ht-degree: 0%
---
# 為何將提示對應至角色，以及您如何為不同對象量身打造提示？

以人物為基礎的提示對應群組會根據實際使用者型別進行提示。 本影片說明為何對您的對象量身打造提示，可揭示您在AI答案中的真實站姿。

## 這部影片是給誰看的？

* 行銷人員和使用者結構化他們追蹤的提示
* 團隊想要對象特定的可見度，而不僅僅是平均值

## 視訊內容

在本影片中，您將瞭解：

* 什麼是角色型提示對應，以及它為什麼與虛構的字元無關
* 如何將提示與您理想的客戶設定檔及其意圖保持一致
* 為什麼個人化提示會顯示內容在哪裡有效而在哪裡無效
* 如何依角色（例如決策者或學生）叢集主題
* 為何取整個對象的可見度平均值會隱藏您的真實身分

>[!VIDEO](https://video.tv.adobe.com/v/3502804/?captions=chi_hant&learn=on){transcript=true}